...
from scikit_tt_gpu.solvers import sle
import time as _time
from scikit_tt_gpu.solvers.sle import __construct_stack_right_op, __construct_stack_left_op, __construct_micro_matrix_als
...
def hod(operator: 'TT', 
    ...
    if op_hod == None:
        ...
        for k in range(2,order//2+1):
            ...
            op_hod = op_hod + 2/cp.math.factorial(2*k-1) * step_size**(2*k-1) * op_tmp
    ...
    for i in range(number_of_steps):
        ...
        if i == 0: # initialize: one expl. Euler and HOD half step backwards in time if previous step is not given
            ...
            if previous_value==None:
                ...
                for k in range(2,order//2+1):
                    op_tmp = op_tmp.dot(operator).dot(operator)
                    op_first = op_first + 2/cp.math.factorial(2*k-1) * (step_size/2)**(2*k-1) * op_tmp
...
def implicit_euler(operator: 'TT', initial_value: 'TT', initial_guess: 'TT', 
                   ...
                   max_rank=cp.infty, micro_solver='solve', normalize=1, progress=True) -> List['TT']:
    """
...
def trapezoidal_rule(operator: 'TT', initial_value: 'TT', initial_guess: 'TT',
                     ...
                     max_rank: int=cp.infty, micro_solver: str='solve',
...
def adaptive_step_size(operator: 'TT', initial_value: 'TT', initial_guess: 'TT', 
    ...
    while (time < time_end) and (closeness_pre > closeness_min) and (step_size > step_size_min):
        ...
        factor_closeness = closeness_tol / cp.abs(closeness_difference)

        # compute new step size
        step_size_new = cp.amin([factor_max, factor_safe * factor_local, factor_safe * factor_closeness]) * step_size

        # accept or reject step
        if (factor_local > 1) and (factor_closeness > 1):
            time = cp.min([time + step_size, time_end])
            step_size = cp.amin([step_size_new, time_end - time, step_size_max])
...
def lie_splitting(S : Union[cp.ndarray, List[cp.ndarray]],
                  L : Union[cp.ndarray, List[cp.ndarray]],
                  I : Union[cp.ndarray, List[cp.ndarray]],
                  M : Union[cp.ndarray, List[cp.ndarray]],
                  initial_value: 'TT', step_size: float, number_of_steps: int, 
                  threshold: float=1e-12, max_rank: int=50,
                  normalize: int=1, K: List[cp.ndarray]=None,
    ...
    for i in range(number_of_steps):
        ...
        # Strang splitting
        tmp = __splitting_stage(K, cp.arange(0,order,2), tmp, threshold, tmp_rank)
        tmp = __splitting_stage(K, cp.arange(1,order,2), tmp, threshold, tmp_rank)
...
def strang_splitting(S : Union[cp.ndarray, List[cp.ndarray]],                     
                     L : Union[cp.ndarray, List[cp.ndarray]],
                     I : Union[cp.ndarray, List[cp.ndarray]],
                     M : Union[cp.ndarray, List[cp.ndarray]],
                     initial_value: 'TT', step_size: float, number_of_steps: int, 
                     threshold: float=1e-12, max_rank: int=50,
                     normalize: int=0, K: List[cp.ndarray]=None) -> List['TT']:
    ...
    for i in range(number_of_steps):
        ...
        # Strang splitting
        tmp = __splitting_stage(K, cp.arange(0,order,2), tmp, threshold, 2*max_rank)
        tmp = __splitting_stage(K, cp.arange(1,order,2), tmp, threshold, 2*max_rank)
        tmp = __splitting_stage(K, cp.arange(0,order,2), tmp, threshold, 2*max_rank)
        tmp = tmp.ortho(threshold=threshold, max_rank=max_rank)
...
def yoshida_splitting(S : Union[cp.ndarray, List[cp.ndarray]],
                      L : Union[cp.ndarray, List[cp.ndarray]],
                      I : Union[cp.ndarray, List[cp.ndarray]],
                      M : Union[cp.ndarray, List[cp.ndarray]],
                      initial_value: 'TT', step_size: float, number_of_steps: int, 
    ...
    for i in range(number_of_steps):
        ...
        # splitting
        tmp = __splitting_stage(K1, cp.arange(0,order,2), tmp, threshold, 2*max_rank)
        tmp = __splitting_stage(K1, cp.arange(1,order,2), tmp, threshold, 2*max_rank)
        tmp = __splitting_stage(K1, cp.arange(0,order,2), tmp, threshold, 2*max_rank)
        #tmp = tmp.ortho(threshold=threshold, max_rank=max_rank)
        tmp = __splitting_stage(K2, cp.arange(0,order,2), tmp, threshold, 2*max_rank)
        tmp = __splitting_stage(K2, cp.arange(1,order,2), tmp, threshold, 2*max_rank)
        tmp = __splitting_stage(K2, cp.arange(0,order,2), tmp, threshold, 2*max_rank)
        #tmp = tmp.ortho(threshold=threshold, max_rank=max_rank)
        tmp = __splitting_stage(K1, cp.arange(0,order,2), tmp, threshold, 2*max_rank)
        tmp = __splitting_stage(K1, cp.arange(1,order,2), tmp, threshold, 2*max_rank)
...
def kahan_li_splitting(S : Union[cp.ndarray, List[cp.ndarray]],
                       L : Union[cp.ndarray, List[cp.ndarray]],
                       I : Union[cp.ndarray, List[cp.ndarray]],
                       M : Union[cp.ndarray, List[cp.ndarray]],
                       initial_value: 'TT', step_size: float, number_of_steps: int, 
    ...
    for i in range(number_of_steps):
        ...
        # splitting
        for j in range(9):
            tmp = __splitting_stage(K[j], cp.arange(0,order,2), tmp, threshold, 2*max_rank)
            tmp = __splitting_stage(K[j], cp.arange(1,order,2), tmp, threshold, 2*max_rank)
            tmp = __splitting_stage(K[j], cp.arange(0,order,2), tmp, threshold, 2*max_rank)
            #tmp = tmp.ortho(threshold=threshold, max_rank=max_rank)
...
def __splitting_propagators(S: Union[cp.ndarray, List[cp.ndarray]],
                            L: Union[cp.ndarray, List[cp.ndarray]],
                            I: Union[cp.ndarray, List[cp.ndarray]],
                            M: Union[cp.ndarray, List[cp.ndarray]],
    ...
    if isinstance(S, list):
        ...
        for i in range(order-1):
            ...
            K[i] = cp.kron(S[i], I[i+1]) + cp.einsum('ijk, klm -> iljm', L[i], M[i+1]).reshape([L[i].shape[0]*M[i+1].shape[1], L[i].shape[1]*M[i+1].shape[2]])
        ...
        K_hom = cp.kron(S, I) + cp.einsum('ijk, klm -> iljm', L, M).reshape([L.shape[0]*M.shape[1], L.shape[1]*M.shape[2]])
...
def __splitting_stage(K: Union[cp.ndarray, List[cp.ndarray]],
                      indices: cp.ndarray, tmp, 
    ...
    for i in indices:
        ...
        if i<tmp.order-1:
            ...
            # contract cores
            tmp_vec = cp.einsum('ijkl,lmno -> ijkmno', tmp.cores[i], tmp.cores[i+1]).reshape([tmp.ranks[i], tmp.row_dims[i]*tmp.row_dims[i+1], tmp.ranks[i+2]])
            tmp_vec = cp.einsum('ijk, lj -> ilk', tmp_vec, K[i]).reshape([tmp.ranks[i]*tmp.row_dims[i], tmp.row_dims[i+1]*tmp.ranks[i+2]])
            ...
            tmp.cores[i+1] = (cp.dot(cp.diag(s),v)).reshape([u.shape[1], tmp.row_dims[i+1], 1, tmp.ranks[i+2]])
            tmp.ranks[i+1] = u.shape[1]

        else:

            tmp.cores[-1] = cp.einsum('ijkl, mj -> imkl', tmp.cores[-1], K[-1])
...
def __update_core_tdvp(i: int, micro_op: cp.ndarray, solution: 'TT', step_size: float, direction: str):
    """
    ...
    if direction == 'forward':
        ...
        if i < solution.order-1:
            ...
            # adapt micro matrix
            q = cp.tensordot(q, cp.eye(r2),axes=0)
            q = q.transpose([0,3,1,2]).reshape([r1*n*r2, solution.ranks[i + 1]*r2])
            micro_op = cp.conj(q).T@micro_op@q
            ...
            solution.cores[i+1] = cp.tensordot(r, solution.cores[i+1], axes=(1,0))
    ...
    if direction == 'backward':
        ...
        if i > 0:
            ...
            # adapt micro matrix
            q = cp.tensordot(cp.eye(r1),q,axes=0)
            ...
            # save non-orthonormal part
            solution.cores[i-1] = cp.tensordot(solution.cores[i-1], r, axes=(3,0))
...
def krylov(operator: 'TT', initial_value: 'TT', dimension: int, step_size: float, threshold: float=1e-12, max_rank: int=50, normalize: int=0) -> 'TT':
    ...
    T = cp.zeros([dimension, dimension], dtype=complex)
    ...
    w_tmp = cp.zeros([dimension], dtype=complex)
    w_tmp[0] = 1
