...
def slim_mme(state_space: List[int], 
    ...
    for i in range(len(state_space)):
        i_mat.append(cp.eye(state_space[i]))
    ...
    for i in range(len(state_space)):
        ...
        s_mat.append(cp.zeros([state_space[i]] * 2))
        ...
        for j in range(len(single_cell_reactions[i])):
            ...
            s_mat[i] = s_mat[i] + reaction_rate * (cp.eye(dimension, k=-net_change) - cp.eye(dimension)).dot(cp.diag(
                cp.eye(dimension)[:, reactant_state]))
    ...
    for i in range(len(state_space) - 1):
        ...
        super_core = cp.zeros([state_space[i]] * 2 + [state_space[i + 1]] * 2)
        ...
        for j in range(len(two_cell_reactions[i])):
            ...
            super_core = super_core + reaction_rate * (cp.tensordot(
                cp.eye(dimension_1, k=-net_change_1).dot(cp.diag(cp.eye(dimension_1)[:, reactant_state_1])),
                cp.eye(dimension_2, k=-net_change_2).dot(cp.diag(cp.eye(dimension_2)[:, reactant_state_2])), axes=0)
                                                       - cp.tensordot(
                        cp.diag(cp.eye(dimension_1)[:, reactant_state_1]),
                        cp.diag(cp.eye(dimension_2)[:, reactant_state_2]), axes=0))
    ...
    if len(two_cell_reactions) == len(state_space):
        ...
        # define super-core as a 4-dimensional tensor of all zeros
        super_core = cp.zeros([state_space[-1]] * 2 + [state_space[0]] * 2)
    ...
    cores = [cp.zeros([1, state_space[0], state_space[0], 2 + ranks[0] + ranks[-1]])]
    ...
    for i in range(1, len(state_space) - 1):
        cores.append(cp.zeros([2 + ranks[i - 1] + ranks[-1], state_space[i], state_space[i], 2 + ranks[i] + ranks[-1]]))
        cores[i][0, :, :, 0]                             = i_mat[i]
    ...
    cores.append(cp.zeros([2 + ranks[-2] + ranks[-1], state_space[-1], state_space[-1], 1]))
...
def __slim_tcr_decomposition(super_core: cp.ndarray, threshold: float) -> Tuple[cp.ndarray, cp.ndarray, int]:
    """
    ...
    Returns
    -------
    core_left : cp.ndarray
        TT core for first cell
    core_right : cp.ndarry
    ...
    rank = u.shape[1]
    core_left = (u.dot(cp.diag(s))).reshape(dimension_1, dimension_1, rank)
