...
from scikit_tt_gpu.tensor_train import TT # pip install git+https://github.com/PGelss/scikit_tt
...
def sampling(quantum_state: 'TT', measure_list: List[float], number_of_samples: int, plot_tf: bool=False):
    ...
    sample_matrix = cp.zeros([number_of_samples, len(measure_list)])

    # generate random numbers in [0,1] for faster sampling
    samples = cp.random.rand(number_of_samples, len(measure_list))
    ...
    for i in range(len(measure_list)):
        cores_tmp[i] = probabilities.cores[i][:,:,0,:]@(cp.eye(int(cp.sqrt(probabilities.ranks[i+1]))).flatten())

    ## sampling
    
    # left part of the network
    theta = cp.ones([number_of_samples,1])
    ...
    for i in range(probabilities.order):
        ...
        sample_matrix[:,i] = (samples[:,i]>cond_prob[:,0]/cp.sum(cond_prob,axis=1))
        
        # update left part of the network
        theta = cp.einsum('ij,jil->il', theta, probabilities.cores[i][:,sample_matrix[:,i].astype('int'),0,:])
        
    # sort outcomes and compute probabilities
    samples, counts = cp.unique(sample_matrix, return_counts=True, axis=0)
...
def plot_histogram(samples, probabilities):
    ...
    plt.ylim([0,cp.max(probabilities)*1.2])
    plt.ylabel('\\textsf{probabilities}', fontsize=16)
    plt.xlabel('\\textsf{measurements}', fontsize=16)
    tick_label=[cp.array2string(samples[i,:].astype(int), separator='')[-2:0:-1] for i in range(samples.shape[0])]
    for i, v in enumerate(probabilities):
        plt.text(i, v+0.01, str('%.3f' %v), horizontalalignment='center', fontsize=12)
    plt.bar(cp.arange(samples.shape[0]), probabilities, tick_label=tick_label)
    plt.tight_layout()
    plt.show()
