...
import cupyx.scipy.sparse.linalg as splin

from typing import List, Union, Tuple

import scikit_tt_gpu.tensor_train as tt
import scikit_tt_gpu.solvers.sle as sle
...
def als(operator: 'TT',
    ...
    shift : float, optional
        shift parameter for known eigecpairs
    ...
    eigenvalues_pre = cp.array([cp.infty]*number_ev)[None,:]
    conv_tf = False

    # initialize variables for optimal eigecpair (number_ev=1)
    eigenvalue_opt  = cp.infty
    ...
    while current_iteration <= repeats and not conv_tf:
        ...
        # save optimal eigecpair
        if number_ev == 1:

            if cp.abs(eigenvalues[0]-sigma)<cp.abs(eigenvalue_opt-sigma):
                eigenvalue_opt = eigenvalues[0].copy()
                eigentensor_opt = TT([trains.solution.cores[0][:, :, :, :, 0]] + trains.solution.cores[1:])

        # check for convergence
        last = eigenvalues_pre[-cp.amin([3, eigenvalues_pre.shape[0]]):, :]

        # last_rel_diff = cp.abs(cp.dot(last - eigenvalues, cp.diag(cp.reciprocal(eigenvalues))))
        last_diff = cp.abs(last - eigenvalues)

        if cp.amax(last_diff)<conv_eps:
            conv_tf = True
        eigenvalues_pre = cp.vstack((eigenvalues_pre, eigenvalues))
...
def __construct_left_stacks(i: int, trains, stacks):
    ...
    if i == 0:
        ...
        stacks.op_left[i] = cp.array([1], ndmin=3)

        if trains.operator_gevp is not None:
            stacks.op_gevp_left[i] = cp.array([1], ndmin=3)

        for j in range(len(trains.previous)):
            stacks.previous_left[j][i] = cp.array([1], ndmin=2)

    else:

        # contract previous stack element with solution and operator cores
        stacks.op_left[i] = cp.tensordot(stacks.op_left[i - 1], trains.solution.cores[i - 1][:, :, 0, :], axes=(0, 0))
        stacks.op_left[i] = cp.tensordot(stacks.op_left[i], trains.operator.cores[i - 1], axes=([0, 2], [0, 2]))
        stacks.op_left[i] = cp.tensordot(stacks.op_left[i], trains.solution.cores[i - 1][:, :, 0, :], axes=([0, 2], [0, 1]))

        if trains.operator_gevp is not None:
            stacks.op_gevp_left[i] = cp.tensordot(stacks.op_gevp_left[i - 1], trains.solution.cores[i - 1][:, :, 0, :], axes=(0, 0))
            stacks.op_gevp_left[i] = cp.tensordot(stacks.op_gevp_left[i], trains.operator_gevp.cores[i - 1], axes=([0, 2], [0, 2]))
            stacks.op_gevp_left[i] = cp.tensordot(stacks.op_gevp_left[i], trains.solution.cores[i - 1][:, :, 0, :], axes=([0, 2], [0, 1]))

        for j in range(len(trains.previous)):
            stacks.previous_left[j][i] = cp.tensordot(stacks.previous_left[j][i - 1], trains.previous[j].cores[i - 1][:, :, 0, :], axes=(0, 0))
            stacks.previous_left[j][i] = cp.tensordot(stacks.previous_left[j][i], trains.solution.cores[i - 1][:, :, 0, :], axes=([0, 1], [0, 1]))
...
def __construct_right_stacks(i: int, trains, stacks):
    ...
    if i == trains.operator.order - 1:
        ...
        stacks.op_right[i] = cp.array([1], ndmin=3)

        if trains.operator_gevp is not None:
            stacks.op_gevp_right[i] = cp.array([1], ndmin=3)

        for j in range(len(trains.previous)):
            stacks.previous_right[j][i] = cp.array([1], ndmin=2)

    else:

        # contract previous stack element with solution and operator cores
        stacks.op_right[i] = cp.tensordot(trains.solution.cores[i + 1][:, :, 0, :], stacks.op_right[i + 1], axes=(2, 2))
        stacks.op_right[i] = cp.tensordot(trains.operator.cores[i + 1], stacks.op_right[i], axes=([1, 3], [1, 3]))
        stacks.op_right[i] = cp.tensordot(trains.solution.cores[i + 1][:, :, 0, :], stacks.op_right[i], axes=([1, 2], [1, 3]))

        if trains.operator_gevp is not None:
            stacks.op_gevp_right[i] = cp.tensordot(trains.solution.cores[i + 1][:, :, 0, :], stacks.op_gevp_right[i + 1], axes=(2, 2))
            stacks.op_gevp_right[i] = cp.tensordot(trains.operator_gevp.cores[i + 1], stacks.op_gevp_right[i], axes=([1, 3], [1, 3]))
            stacks.op_gevp_right[i] = cp.tensordot(trains.solution.cores[i + 1][:, :, 0, :], stacks.op_gevp_right[i], axes=([1, 2], [1, 3]))

        for j in range(len(trains.previous)):
            stacks.previous_right[j][i] = cp.tensordot(trains.solution.cores[i + 1][:, :, 0, :], stacks.previous_right[j][i + 1], axes=(2, 1))   
            stacks.previous_right[j][i] = cp.tensordot(trains.previous[j].cores[i + 1][:, :, 0, :], stacks.previous_right[j][i], axes=([1, 2], [1, 2]))


def __construct_micro_matrices(i: int, trains, stacks, shift: float) -> cp.ndarray:
    """
    ...
    micro_op = cp.tensordot(stacks.op_left[i], trains.operator.cores[i], axes=(1, 0))
    micro_op = cp.tensordot(micro_op, stacks.op_right[i], axes=(4, 1))
    ...
    if trains.operator_gevp is not None:
        micro_op_gevp = cp.tensordot(stacks.op_gevp_left[i], trains.operator_gevp.cores[i], axes=(1, 0))
        micro_op_gevp = cp.tensordot(micro_op_gevp, stacks.op_gevp_right[i], axes=(4, 1))
    ...
    for j in range(len(trains.previous)):
        tmp = cp.tensordot(stacks.previous_left[j][i], trains.previous[j].cores[i][:, :, 0, :], axes=(0, 0))
        tmp = cp.tensordot(tmp, stacks.previous_right[j][i], axes=(2, 0))
...
def __update_core(i: int, micro_op: cp.ndarray, 
                  micro_op_gevp: 'TT', 
    ...
    if solver == 'eig':
        ...
        idx = (cp.abs(eigenvalues - sigma)).argsort()
    ...
    if direction == 'forward':
        ...
        # rank reduction
        r = cp.minimum(solution.ranks[i + 1], u.shape[1])
        u = u[:, :r]
    ...
    if direction == 'backward':
        ...
        if i > 0:
            ...
            # rank reduction
            r = cp.minimum(solution.ranks[i], v.shape[0])
            v = v[:r, :]
