...
def mals(operator: 'TT', initial_guess: 'TT', right_hand_side: 'TT', repeats: int=1, solver: str='solve', threshold: float=1e-12, max_rank: int=cp.infty) -> 'TT':
    """
...
def __construct_stack_left_op(i: int, stack_left_op: List[cp.ndarray], operator: 'TT', solution: 'TT'):
    """
    ...
    if i == 0:
        ...
        stack_left_op[i] = cp.array([1], ndmin=3)
    else:

        # contract previous stack element with solution and operator cores
        stack_left_op[i] = cp.tensordot(stack_left_op[i - 1], solution.cores[i - 1][:, :, 0, :], axes=(0, 0))
        stack_left_op[i] = cp.tensordot(stack_left_op[i], operator.cores[i - 1], axes=([0, 2], [0, 2]))
        stack_left_op[i] = cp.tensordot(stack_left_op[i], cp.conj(solution.cores[i - 1][:, :, 0, :]), axes=([0, 2], [0, 1]))
...
def __construct_stack_left_rhs(i, stack_left_rhs, right_hand_side, solution):
    ...
    i : int
        core index
    stack_left_rhs : list[cp.ndarray]
    ...
    if i == 0:
        ...
        # first stack element is 1
        stack_left_rhs[i] = cp.array([1], ndmin=2)

    else:

        # contract previous stack element with solution and right-hand side cores
        stack_left_rhs[i] = cp.tensordot(stack_left_rhs[i - 1], right_hand_side.cores[i - 1][:, :, 0, :], axes=(0, 0))
        stack_left_rhs[i] = cp.tensordot(stack_left_rhs[i], cp.conj(solution.cores[i - 1][:, :, 0, :]), axes=([0, 1], [0, 1]))


def __construct_stack_right_op(i: int, stack_right_op: List[cp.ndarray], operator: 'TT', solution: 'TT'):
    """
    ...
    i : int
        core index
    stack_right_op : list[cp.ndarray]
    ...
    if i == operator.order - 1:
        ...
        stack_right_op[i] = cp.array([1], ndmin=3)

    else:

        # contract previous stack element with solution and operator cores
        stack_right_op[i] = cp.tensordot(cp.conj(solution.cores[i + 1][:, :, 0, :]), stack_right_op[i + 1], axes=(2, 2))
        stack_right_op[i] = cp.tensordot(operator.cores[i + 1], stack_right_op[i], axes=([1, 3], [1, 3]))
        stack_right_op[i] = cp.tensordot(solution.cores[i + 1][:, :, 0, :], stack_right_op[i], axes=([1, 2], [1, 3]))
...
def __construct_stack_right_rhs(i, stack_right_rhs, right_hand_side, solution):
    ...
    i : int
        core index
    stack_right_rhs : list[cp.ndarray]
    ...
    if i == right_hand_side.order - 1:
        ...
        # last stack element is 1
        stack_right_rhs[i] = cp.array([1], ndmin=2)

    else:

        # contract previous stack element with solution and right-hand side cores
        stack_right_rhs[i] = cp.tensordot(cp.conj(solution.cores[i + 1][:, :, 0, :]), stack_right_rhs[i + 1], axes=(2, 1))
        stack_right_rhs[i] = cp.tensordot(right_hand_side.cores[i + 1][:, :, 0, :], stack_right_rhs[i],
                                          axes=([1, 2], [1, 2]))
...
def __construct_micro_matrix_als(i: int, 
                                 stack_left_op:  List[cp.ndarray], 
                                 stack_right_op: List[cp.ndarray], 
                                 operator: 'TT', solution: 'TT') -> cp.ndarray:
    """
    ...
    micro_op = cp.tensordot(stack_left_op[i], operator.cores[i], axes=(1, 0))
    micro_op = cp.tensordot(micro_op, stack_right_op[i], axes=(4, 1))
...
def __construct_micro_matrix_mals(i: int, 
    ...
    micro_op = cp.tensordot(stack_left_op[i], operator.cores[i], axes=(1, 0))
    micro_op = cp.tensordot(micro_op, operator.cores[i + 1], axes=(4, 0))
    micro_op = cp.tensordot(micro_op, stack_right_op[i + 1], axes=(6, 1))
...
def __construct_micro_rhs_als(i: int, 
                              stack_left_rhs:  List[cp.ndarray], 
                              stack_right_rhs: List[cp.ndarray], 
                              right_hand_side: 'TT', solution: 'TT') -> cp.ndarray:
    ...
    micro_rhs = cp.tensordot(stack_left_rhs[i], right_hand_side.cores[i][:, :, 0, :], axes=(0, 0))
    micro_rhs = cp.tensordot(micro_rhs, stack_right_rhs[i], axes=(2, 0))
...
def __construct_micro_rhs_mals(i: int, 
    ...
    micro_rhs = cp.tensordot(stack_left_rhs[i], right_hand_side.cores[i][:, :, 0, :], axes=(0, 0))
    micro_rhs = cp.tensordot(micro_rhs, right_hand_side.cores[i + 1][:, :, 0, :], axes=(2, 0))
    micro_rhs = cp.tensordot(micro_rhs, stack_right_rhs[i + 1], axes=(3, 0))
...
def __update_core_als(i: int, 
                      micro_op: cp.ndarray, micro_rhs: cp.ndarray, 
    ...
    if solver == 'solve':
        solution.cores[i] = cp.linalg.solve(micro_op, micro_rhs)
    if solver == 'lu':
    ...
    if direction == 'backward':
        ...
        if i > 0:
            ...
            # decompose solution
            A = solution.cores[i].reshape(solution.ranks[i], solution.row_dims[i] * solution.ranks[i + 1])
            P = cp.fliplr(cp.identity(solution.ranks[i]))
            P_prime = cp.fliplr(cp.identity(int(cp.amin(cp.array([solution.ranks[i], solution.row_dims[i] * solution.ranks[i + 1]])))))
            [Q, _] = cp.linalg.qr(
                (P @ A).T,
                mode='reduced')
            q = P_prime @ Q.T
...
def __update_core_mals(i: int, 
    ...
    if direction == 'forward':
        ...
        # decompose solution
        [u, s, _] = cp.linalg.svd(solution.cores[i].reshape(solution.ranks[i] * solution.row_dims[i],
    ...
    if direction == 'backward':
        ...
        # decompose solution
        [u, s, v] = cp.linalg.svd(solution.cores[i].reshape(solution.ranks[i] * solution.row_dims[i],
                                                      solution.row_dims[i + 1] * solution.ranks[i + 2]),
                                  full_matrices=False)
        ...
        if i == 0:
            # last iteration step
            solution.cores[i] = (u.dot(cp.diag(s))).reshape(solution.ranks[i], solution.row_dims[i], 1,
                                                            solution.ranks[i + 1])
